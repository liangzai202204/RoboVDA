...
state_report_frequency = {config['state_report_frequency']}
                ...
                os.system(f"echo seer | sudo -S systemctl enable startup_vda.service")
                os.system(f"echo seer | sudo -S systemctl start my_service.service")
                # 更新状态和日志
                self.status = MoveStatus.FINISHED
            elif args["operation"] == "creat_start_script":
                if not os.path.exists(os.path.join("/usr/local/SeerRobotics/vda", 'start_script.sh')):
                    with open(os.path.join("/usr/local/SeerRobotics/vda", 'start_script.sh'), 'w') as file:
                        file.write(START_SCRIPT)
                os.chmod(os.path.join("/usr/local/SeerRobotics/vda", 'start_script.sh'),
                         stat.S_IRWXU |stat.S_IRWXG | stat.S_IRWXO)
                os.chmod(os.path.join("/usr/local/SeerRobotics/vda", 'app'),
                         stat.S_IRWXU |stat.S_IRWXG | stat.S_IRWXO)
...
ConditionPathExists=/usr/local/SeerRobotics/vda/start_script.sh
After=network.target

[Service]
ExecStart=/usr/local/SeerRobotics/vda/start_script.sh
Restart=always
RestartSec=3
...
echo seer | sudo -S ./app  # 运行 Python 程序
"""
